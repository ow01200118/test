...
IOTZ_RETURN iotz_write_mct_req(
    IOTZ_FILE* fp,
    IOTZ_UBYTE* key,
    IOTZ_INT keyLen,
    IOTZ_UBYTE* iv,
...
IOTZ_RETURN iotz_write_mct_fax(
    IOTZ_FILE* fp,
    IOTZ_UBYTE* key,
    IOTZ_INT keyLen,
    IOTZ_UBYTE* iv,
...
)
{
    ...
    for (i = 0; i < IOTZ_MCT_TEST_COUNT; i++)
    {
        ...
        {
            blockcipher_ecb_enc(ct, &ctLen, tPt, ptLen, tKey, keyLen, alg);
        ...
        iotz_write_mct_fax(fp, tKey, keyLen >> 3, NULL, 0, tPt, ptLen, ct, ctLen, IOTZ_ECB, i);
...
)
{
    ...
    sprintf_s(fileName, FILE_NAME_SIZE, "%s%sMCT.fax", PREFIX_FILE_PATH, fName);
    fopen_s(&fp2, fileName, "wt");
#else
    sprintf(fileName, "%s%sMCT.req", PREFIX_FILE_PATH, fName);
    fp1 = fopen(fileName, "wt");
    sprintf(fileName, "%s%sMCT.fax", PREFIX_FILE_PATH, fName);
    ...
    iotz_write_mct_req(fp1, key, keyLen, iv, ivLen, pt, ptLen, mode);
    ...
    {
    case IOTZ_ECB:
        iotz_mct_ecb_fax(fp2, key, keySize, pt, ptLen, alg);
        break;
    case IOTZ_CBC:
        break;
    case IOTZ_CFB1:
    ...
    print_log("    Generate MCT Request \'%sMCT.req\' \'%sMCT.fax\' done", fName, fName);
