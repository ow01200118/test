...
)
{
    ...
    if (fscanf_s(fp, "KEY = %s\n", buf, BUF_SIZE) <= 0)
		return IOTZ_MMT_FILE_READ_ERROR;
#else
    if (fscanf(fp, "KEY = %s\n", buf) <= 0)
        return IOTZ_MMT_FILE_READ_ERROR;
#endif
...
IOTZ_RETURN iotz_write_mmt_rsp(
    IOTZ_FILE* fp,
    IOTZ_UBYTE* key,
    IOTZ_INT keyLen,
    IOTZ_UBYTE* iv,
...
)
{
    ...
    while (!feof(fp1))
    {
        ...
        iotz_read_mmt_req(fp1, key, &keyLen, iv, &ivLen, pt, &ptLen, mode);

        query_blockcipher_enc(ct, &ctLen, pt, ptLen, iv, ivLen, key, keyLen, mode, alg);

        iotz_write_mmt_rsp(fp2, key, keyLen, iv, ivLen, pt, ptLen, ct, ctLen, mode);
    }

    fclose(fp1);
    fclose(fp2);

    print_log("    Generate MMT Request \'%sMMT.rsp\' done", fName);
